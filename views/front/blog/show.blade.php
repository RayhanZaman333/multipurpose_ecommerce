...
@section('meta')
    ...
    <meta name="twitter:image" content="{{ asset('storage/images/' . json_decode($post->photo, true)[0]) }}">
    <meta name="twitter:description" content="{{ $post->meta_descriptions }}">

    <meta name="og:title" content="{{ $post->title }}">
    <meta name="og:image" content="{{ asset('storage/images/' . json_decode($post->photo, true)[0]) }}">
                ...
                <div class="card blog-details-box">
                    ...
                    <div class="blog-details-slider owl-carousel">
                        ...
                        @foreach (json_decode($post->photo, true) as $photo)
                            <img src="{{ asset('storage/images/' . $photo) }}" alt="Image">
                        @endforeach
                    </div>
                            ...
                            <div class="widget widget-featured-posts">
                                <div class="entry">
                                    <div class="entry-thumb"><a
                                            href="{{ route('front.blog.details', $like_post->slug) }}"><img
                                                src="{{ asset('storage/images/' . json_decode($like_post->photo, true)[array_key_first(json_decode($like_post->photo, true))]) }}"
                                                alt="Post"></a></div>
                                    <div class="entry-content">
                                        <h4 class="entry-title"><a
                                                href="{{ route('front.blog.details', $like_post->slug) }}">
                                                {{ Str::limit($like_post->title, 75) }}
                                            </a></h4><span class="entry-meta">{{ __('by') }}
                                            {{ __('Admin') }}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        @foreach ($posts as $recent)
                            <div class="entry">
                                <div class="entry-thumb"><a href="{{ route('front.blog.details', $recent->slug) }}"><img
                                            src="{{ asset('storage/images/' . json_decode($recent->photo, true)[array_key_first(json_decode($recent->photo, true))]) }}"
                                            alt="Post"></a></div>
                                <div class="entry-content">
