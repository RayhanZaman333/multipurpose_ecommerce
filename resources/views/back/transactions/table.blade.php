@foreach($datas as $data)
    <tr id="transaction-bulk-delete">
        <td><input type="checkbox" class="form-control bulk-item" value="{{ $data->id }}"></td>

        <td >
            @if (!$data->user->id)
                {{ $data->user_email }}
            @else
                <a href="{{ route('back.user.show', $data->user->id) }}"><i class="fa fa-eye"></i> {{ $data->user_email }}</a>
            @endif
        </td>

        <td>
            <a href="{{ route('back.order.invoice', $data->order_id) }}"><i class="fa fa-eye"></i> {{ $data->txn_id }}</a>
        </td>
