                        ...
                        <tr>
                            <td>
                                <div class="product-item">
                                    <a class="product-thumb" href="{{ route('front.product', $item['slug']) }}"><img src="{{ asset('storage/images/' . $item['photo']) }}" alt="Product"></a>

                                    <div class="product-info">
                                        <h4 class="product-title">
                                            <a href="{{ route('front.product', $item['slug']) }}"> {{ Str::limit($item['name'], 45) }}</a>
                                        </h4>
                            ...
                            <td class="text-center text-lg">{{ PriceHelper::setCurrencyPrice($item['main_price'] + $item['attribute_price']) }}</td>
                            ...
                            <td class="text-center text-lg">{{ PriceHelper::setCurrencyPrice(($item['main_price'] + $item['attribute_price']) * $item['qty']) }}</td>
    ...
    <div class="card-body">
        <div class="shopping-cart-footer">
            ...
            <div class="text-right text-lg column {{ Session::has('coupon') ? '' : 'd-none' }}">
                ...
                <a class="remove-from-cart btn btn-danger btn-sm" href="{{ route('front.promo.destroy') }}" data-toggle="tooltip" title="Remove item"><i class="icon-x"></i></a>
            </div>
        ...
        <div class="shopping-cart-footer">
            <div class="column">
                <a class="btn btn-primary " href="{{ route('front.catalog') }}">
                    <span><i class="icon-arrow-left"></i> {{ __('Back to Shopping') }}</span>
                </a>
            </div>
